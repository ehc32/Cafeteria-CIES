...
*
* @author: Jeevan Lal - Edited by Yamir H. 2022
* @version: 0.0.5
*/
...
class Authorization_Token 
{
     ...
     * Token Expire Time
     * ----------------------
     * ( 1 Day ) : 60 * 60 * 24 = 86400 / Segundos * Minutos * Horas
     * ( 1 Hour ) : 60 * 60     = 3600
     */
    protected $token_expire_time = 60 * 60 * 24; 
    ...
    public function validateToken()
    {
        ...
        if($token_data['status'] === TRUE)
        {
            try
            {
                 ...
                 */
                try {
                    $token_decode = JWT::decode($token_data['key'], new Key($this->token_key, $this->token_algorithm));
                }
                catch(Exception $e) {
        ...
        if(isset($_POST['token']))
        {
            ...
            if(!empty($token) AND is_string($token) AND !is_array($token))
            {
                try
                {
                     ...
                     */
                    try {
                        $token_decode = JWT::decode($token, new Key($this->token_key, $this->token_algorithm));
                    }
                    catch(Exception $e) {
                        return ['status' => FALSE, 'message' => $e->getMessage()];
                    }
        ...
        if(!empty($headers) AND is_array($headers)) {
            foreach ($this->token_header as $key) {
                if (array_key_exists($key, $headers) AND !empty($key))
                    return ['status' => TRUE, 'key' => $headers[$key]];
            }
        }
        return ['status' => FALSE, 'message' => 'Token is not defined.'];
    }
